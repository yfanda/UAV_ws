...
namespace grampcd
{
	...
	public:
		...
		void simulate_UAV(typeRNum* ui);


		/*Help function to update x and u reference*/
		void lineReader(double* xdes, int num_cols, const std::string& filename, int line_number);
		void desReader(double* xdes, int NX, const std::string& filename, int line_number, double Thor, double dt);
		void setFilename(const std::string& identifier, const std::string& filename);
		...
		virtual void print_solution_to_file(const unsigned int agent_id, std::string prefix = "Solution_agent") const override;
		/*Print solutions of a set of agents to a file.*/
		virtual void print_solution_to_file(const std::string& agents, std::string prefix = "Solution_agent") const override;
	 ...
	 private:
		/*Run a centralized controller.*/
		void run_MPC_UAV(const std::vector<AgentPtr>& agents, const SimulatorPtr& simulator, const OptimizationInfo& oi, const typeRNum Tsim, const typeRNum t_0, int iMPC, typeRNum* x_next);
		/*Run a centralized controller in endless model*/
		...
		std::string x_traj_file;
		std::string u_traj_file;
