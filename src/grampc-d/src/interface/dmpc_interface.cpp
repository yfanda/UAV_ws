...
namespace grampcd
{
	...
	void DmpcInterface::simulate_UAV(typeRNum* ui)
	{
		const std::vector<AgentPtr>& agents = agents_; 
		const SimulatorPtr& simulator = simulator_;
		const OptimizationInfo& oi = optimizationInfo_;
		...
		simulator->centralized_simulation(x_next, &solver, oi.COMMON_Integrator_, oi.COMMON_dt_,ui);


	}

	void DmpcInterface::run_MPC_UAV
	(
	...
	)
	{
		...
		int n = ceil(optimizationInfo_.COMMON_dt_/0.01);
		int num_xtraj = (optimizationInfo_.COMMON_Thor_/optimizationInfo_.COMMON_dt_ +1)*9;  
		int num_utraj = (optimizationInfo_.COMMON_Thor_/optimizationInfo_.COMMON_dt_ +1)*8;
		typeRNum x_traj[num_xtraj];
		typeRNum u_traj[num_utraj];

		
		
		desReader(x_traj, 9, x_traj_file, iMPC*n+1, optimizationInfo_.COMMON_Thor_, optimizationInfo_.COMMON_dt_);
		desReader(u_traj, 8, u_traj_file, iMPC*n+1, optimizationInfo_.COMMON_Thor_, optimizationInfo_.COMMON_dt_);
		for (const auto& agent : agents)
			agent->update_modelparam(x_traj,num_xtraj,u_traj,num_utraj);
		...
		// rotor1 fails
		// solver.solver_.sol.unext
		// simulator->set_u_fault(&solver);
		...
		{
			*(&state->u_[0] +1) =0;
			// ui = state->u_.empty() ? nullptr : &state->u_[0];
			std::copy(state->u_.begin(), state->u_.begin() + 8, ui);
		}
...
/* 
	...
	/*Help function to update x and u reference*/
	void DmpcInterface::lineReader(double* xdes, int num_cols, const std::string& filename, int line_number)
	{
		std::string line;
        ...
        while (std::getline(file, line)) {
            ...
            if (line_count == line_number) {
                ...
                int num_read = 0;
                while (iss >> xdes[num_read] && num_read < num_cols) {
                    num_read++;
                }
                break; 
            }
	...
	void DmpcInterface::desReader(double* xdes, int NX, const std::string& filename, int line_number, double Thor, double dt)
	{
		int horizon = static_cast<int>(Thor / dt) + 1;
		int n = static_cast<int>(dt / 0.01);
		for (int m = 0; m < horizon; m++) {
			lineReader(xdes + m * NX, NX, filename, line_number + m * n);
		}
	}

	void DmpcInterface::setFilename(const std::string& identifier, const std::string& traj_file_path)
	{
		if (identifier == "x_traj_file") {
            x_traj_file = traj_file_path;
        } else if (identifier == "u_traj_file") {
            u_traj_file = traj_file_path;
        } else {
            std::cerr << "Invalid identifier: " << identifier << std::endl;
        }
	}
	...
	{
		run_MPC_UAV(agents_, simulator_, optimizationInfo_, Tsim, t_0, iMPC, x_next);
	}
	...
	void DmpcInterface::print_solution_to_file(const unsigned int agent_id, std::string prefix) const
	{
	...
	void DmpcInterface::print_solution_to_file(const std::string& agents, std::string prefix) const
	{
		// get solutions
		const auto solutions = communication_interface_->get_solution(agents);

		prefix = "/home/evan/grampc-d/examples/coupled_watertanks/res/solution";
