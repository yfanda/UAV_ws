...
namespace grampcd
{
    ...
    void SolverCentral::solve(typeRNum* x_next)
    {
        // auto UAVmodel= agents_-> model_;
        /*
        To Do:
        1. replace "AgentModelPtr model_ " with  "AgentModelPtr model_";
        2. new method in Class Agent to access AgentModel(private).
        3. read new x and u data each time SolverCentral::solve() is called,using the mehtod from Class DMPCinterface.
        */
        ...
        for(const AgentPtr& agent : agents_)
        {
            ...
            for(unsigned int j = 0; j < Nxi; ++j)
            {
                ...
                solver_->getParameters()->x0[x_index + j] = x_next[j];
            }
