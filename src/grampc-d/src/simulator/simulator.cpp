...
namespace grampcd
{
    ...
    void Simulator::centralized_simulation(typeRNum* x_next,const SolverCentral *solver, const std::string& Integrator, typeRNum dt,typeRNum* ui)
    {
        // integration method
        Integrator_ = Integrator;
    ...
    void Simulator::simulate(typeRNum* x_next_out, typeRNum* u_input)
    {
        // evaluate t0
        ...
        if( Integrator_ == "euler" )
        {
            for(const auto& [id, state] : agentStates_)
            {
...

                {
                    ...
                    agent_model->ffct(&x_next[0], state->t_[0], &state->x_[0], u_input);
                }
                ...
                for( const auto& [neighbor_id, neighbor_model] : *couplingModels )
                {
                    ...
                    neighbor_model->ffct( &x_next[0], state->t_[0], &state->x_[0], u_input, &neighbor_state->x_[0], uj );
                }
        ...
        else if( Integrator_ == "heun" )
        {
            ...
            for(const auto& [id, state] : agentStates_)
            {
...

                {
                    ...
                    ctypeRNum* ui = state->u_.empty() ? nullptr : &state->u_[0];
                    // evaluate f_i( x_i, u_i )
                    agent_model->ffct(&((*ptr_to_data)[0]), state->t_[0], &state->x_[0], u_input);
                }
                ...
                for(const auto& [neighbor_id, neighbor_model] : *couplingModels)
                {
...

                    {
                        ...
                        // evaluate f_{ij}( xi, ui, x_j, u_j )
                        neighbor_model->ffct(&((*ptr_to_data)[0]), state->t_[0], &state->x_[0], u_input, &neighbor_state->x_[0], uj);
                    }           
                }
            ...
            for(const auto& [id, state] : agentStates_)
            {
...

                {
                    ...
                    // evaluate f_i( x_i, u_i )
                    agent_model->ffct(&((*ptr_to_data1)[0]), state->t_[0], &((*ptr_to_data0)[0]), u_input);
                }
                ...
                for(const auto& [neighbor_id, neighbor_model] : *couplingModels)
                {
                   ...
                   
                    {
                        ...
                        // evaluate f_{ij}( xi, ui, x_j, u_j )
                        neighbor_model->ffct(&((*ptr_to_data1)[0]), state->t_[0], &((*ptr_to_data0)[0]), u_input, &((*ptr_to_data_of_neighbor)[0]), uj);
                    }
                }
    ...
    void Simulator::set_u_fault(const SolverCentral *solver)
    {
        ...
        {
            const auto agent_model = communication_interface_->get_agentModel( id );
            typeRNum* ui = state->u_.empty() ? nullptr : &state->u_[0];
            *(ui + 1) = 0;
