...
namespace grampcd
{
    ...
    void Agent::update_modelparam(typeRNum* x_traj,int num_xtraj, typeRNum* u_traj, int num_utraj)
    {
        std::copy(x_traj, x_traj + num_xtraj, model_->model_parameters_.begin() + 2);
        std::copy(u_traj, u_traj + num_utraj, model_->model_parameters_.begin() + 2 + num_xtraj);
        model_->x_des_temp.assign(model_->model_parameters_.begin() + 2, model_->model_parameters_.begin() + 2 + num_xtraj);
	    model_->u_des_temp.assign(model_->model_parameters_.begin() + 2 + num_xtraj, model_->model_parameters_.begin() + 2 + num_xtraj + num_utraj);           
    }
